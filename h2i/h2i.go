...
func (app *h2i) Main() error {
	...
	tc, err := netpoll.DialConnection("tcp", hostAndPort, 200*time.Millisecond)
	tc.SetReadTimeout(1 * time.Minute)
	tc.SetIdleTimeout(10 * time.Minute)
	if err != nil {
	...
	app.conn = tc
	app.w = tc.Writer()
	app.framer = http2.NewFramer(tc.Writer(), tc.Reader())
